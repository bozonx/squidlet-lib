...
/**
 * Parses whole url WARNING: be careful with the next type of urls
 *
...
export function parseUrl(rawUrl: string): ParsedUrl {
  ...
  return { ...leftPart, ...rightPart, pathname: right }
}
...
/**
 * Parses value of search param. Pass it already url decoded. It supports arrays
 * and objects in JSON format like:
 *
 * - Param1=[1, true, "str"]
 * - Param1={"a": "str", "b": 5, "c": true} Params which defined without value
 *   will have an empty string as a value according to URLSearchParams:
 ...
 *
 * - Param1=value1&param2&param3=5&param4=true => { param1: 'value1', param2: '',
 *   param3: 5, param4: true }
 ...
 */
export function parseUserPassword(rawStr: string): {
  user?: string
  password?: string
} {
  if (!rawStr) return {}
...
export function splitUrl(url: string): { left?: string; right?: string } {
  ...
  if (splitUrlMatch[3]) {
    ...
    if (splitUrlMatch[1].match(/[:.@]/)) {
      return { left: splitUrlMatch[1] }
    } else if (splitUrlMatch[1].match(/[\/#?&%]/)) {
      // path part
      return { right: splitUrlMatch[1] }
    }
