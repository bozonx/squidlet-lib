...
/**
 * Parse arguments. It supports formats:
 *
 * - -a value
 * - --name value
 * - -a=value
 ...
 *
 * To obviously resolve boolean args put it to the list of "booleanArgs" use:
...
export function parseArgs(
  ...
  // TODO: навеное не правильно - тогда потеряются файлы с пробелами
    ...
    normalized = normalizeArgs(rawArgs.split(' '), booleanParams)
  else if (Array.isArray(rawArgs))
    normalized = normalizeArgs(rawArgs, booleanParams)
  ...
  for (const item of normalized) {
    ...
    if (match) {
      ...
      params[paramName] = !paramValue
        ? // it is a boolean
          booleanParams.includes(paramName)
          ? true
          : paramValue
        : paramValue
    } else {
...
/**
 * It join wrongly slit of arguments. The problem with filenames which contain
 * spaces. It make separate elements which start only from "-". Which are start
 * from any other symbol - it join to the last string. It puts numeric arguments
 * to the end of line
 *
 * @param rawArgs
 * @param booleanParams
...
export function normalizeArgs(
  ...
  for (let i = 0; i < rawArgs.length; i++) {
    ...
    if (trimmed === '--') {
      args = [...args, ...rawArgs.slice(i + 1).map((el: string) => el.trim())]
...
/**
 * In case file names have spaces. it means all the items after --name to other
 * '-' means one value with spaces. The problem is there no way to recognize how
 * many spaces in file names because process.argv splat all the spaces.
 *
 * @param rawArgs
 */
