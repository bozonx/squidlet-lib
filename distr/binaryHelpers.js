 ...
 */
export function isEqualUint8Array(first, second) {
 ...
 */
export function uint8ArrayKeys(arr) {
    const result = [];
 ...
 */
export function withoutFirstItemUint8Arr(arr) {
 ...
 */
export function addFirstItemUint8Arr(arr, itemToAdd) {
    if (!(arr instanceof Uint8Array)) {
        throw new Error(`binaryHelpers.withoutFirstItemUint8Arr: array have to be an Uint8Array`);
    }
    const itemsToAdd = 1;
 ...
 */
export function hexStringToUint8Arr(hex) {
    if (hex.length < 2) {
 ...
 */
export function uint8ArrToHexString(bytesArr) {
    let result = '';
...
export function hexStringToHexNum(hexString) {
    ...
    if (typeof hexString === 'number')
        return hexString;
    else if (typeof hexString === 'undefined' || hexString === '')
        return 0;
    return parseInt(String(hexString), 16);
}
// TODO: test
export function hexNumToString(hexNum) {
    if (typeof hexNum === 'undefined')
        return '0x00';
    else if (typeof hexNum !== 'number')
        throw new Error(`Incorrect type of hexNum`);
    const str = hexNum.toString(16);
    if (str.length === 1) {
        return `0x0${str}`;
    }
    return `0x${str}`;
 ...
 * Number are always 8.
 */
export function byteToString(hexValue) {
 ...
 */
export function byteToBinArr(hexValue) {
    // convert 4 to ""00000100""
    const binStr = byteToString(hexValue);
    // like ["1", "1", "1", "1", "1", "1", "1", "1"]
    const binSplitStr = binStr.split('');
    const result = new Array(BITS_IN_BYTE);
...
export function numToWord(num) {
    let result = int16ToHexString(num);
    if (result.length === 2)
        result = '00' + result;
 ...
 */
export function numToUint8Word(num) {
    const valueWord = numToWord(num);
 ...
 */
export function uint8ToNum(uint8Arr) {
    const hexStr = uint8ArrToHexString(uint8Arr);
 ...
 */
export function uint16ToUint8(arr16) {
    const result = [];
    for (let item of arr16) {
        const bytes = numToUint8Word(item);
        result.push(bytes[0]);
        result.push(bytes[1]);
    }
 ...
 */
export function uint8ToUint16(arr8) {
    const result = [];
 ...
 * 0 > "00000000"
 */
export function int32ToHexString(int32) {
 ...
 */
export function int32ToUint8Arr(int32) {
...
export function bitsToBytes(bits) {
    ...
    const numOfBytes = Math.ceil(bitsCount / 8);
    const result = new Uint8Array(numOfBytes);
    for (let i = 0; i < bitsCount; i++) {
        // number of byte from 0
        const byteNum = Math.floor(i / 8);
        //const positionInByte: number = (Math.floor(i / 8) * 8);
        const positionInByte = i - (byteNum * 8);
 ...
 * @param value
 */
export function updateBitInByte(byte, position, value) {
 ...
 */
export function getBitFromByte(byte, position) {
    if (position < 0 || position >= BITS_IN_BYTE) {
 ...
 */
export function getAsciiNumber(num) {
    if (num < 0 || num > 9) {
...
export function concatUint8Arr(...arrs) {
    let offset = 0;
    const lengths = arrs.map((item) => item.length)
 ...
 */
export function combine2numberToByte(one, two) {
    const oneStr = padStart(one.toString(2), 4, '0');
    const twoStr = padStart(two.toString(2), 4, '0');
    const arr = (twoStr + oneStr).split('');
    return parseInt(arr.join(''), 2);
}
/**
 * Extract 2 4-bit numbers from byte which has been encoded by `combine2numberToByte`
 */
export function extract2NumbersFromByte(byte) {
    const one = byte & 15;
    const two = byte >> 4;
    return [one, two];
}
 ...
 * * bitNum = 25 returns 3
 */
export function getOctetNum(bitNum) {
 ...
 * * bitNum = 25 returns 4
 */
export function howManyOctets(bitCount) {
 ...
 * @param bitNum
 */
export function getBitNumInOctet(bitNum) {
