...
export function parseBody(contentType, body) {
    ...
    if (contentType === 'application/octet-stream') {
        ...
        if (typeof body !== 'string') {
            throw new Error(`parseBody: Incorrect body: content-type is "application/json" ` +
        ...
        if (!isHtml(body)) {
            throw new Error(`parseBody: Incorrect body: content-type is "text/html" but body isn't a html`);
        }
        return body;
    }
    else if (STRING_CONTENT_TYPES.includes(contentType)) {
        if (typeof body !== 'string') {
            throw new Error(`parseBody: Incorrect body: content-type is "${contentType}" ` +
                `and body has to be a string`);
        }
        return body;
    }
 ...
 */
export function prepareBody(contentType, fullBody) {
    if (!contentType) {
    ...
    if (contentType === 'application/octet-stream') {
        ...
        try {
            ...
            throw new Error(`prepareBody: Incorrect body: content-type is application/json ` +
                `but body can't be converted to JSON`);
        }
    }
    else if (contentType === 'text/html') {
        if (!isHtml(fullBody)) {
            throw new Error(`prepareBody: Incorrect body: content-type is "text/html" but body isn't a html`);
        }
        return fullBody;
    }
    else if (STRING_CONTENT_TYPES.includes(contentType)) {
        if (typeof fullBody !== 'string') {
            throw new Error(`prepareBody: Incorrect body: content-type is "${contentType}" ` +
                `and body has to be a string`);
        }
        return fullBody;
    }
 ...
 */
export function resolveBodyType(fullBody) {
    if (typeof fullBody === 'undefined') {
        return;
    }
